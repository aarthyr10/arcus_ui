...
//     const mapQuestionsToRows = (questions: any[]) =>
//         questions.map((q) => ({
//             "Q No": q.question_no,
//             Question: q.question,
//             Answer: q.answer,
...
//     /* ---------------- PDF ---------------- */
//     const generatePDF = (questions: any[]) => {
...
//         autoTable(doc, {
//             startY: 25,
//             head: [["Q No", "Question", "Answer", "Reference"]],
...
//     };
//     const generateExcel = (questions: any[]) => {
//         const rows = mapQuestionsToRows(questions);
...
//         setTimeout(() => {
//             if (type === "pdf") {
//                 generatePDF(apiResponse.data.questions);
//             }

//             if (type === "excel") {
//                 generateExcel(apiResponse.data.questions);
//             }
...
//                             Generating {exportType === "pdf" ? "PDF" : "Excel"} report...
//                         </p>
//                     </div>
//                 )}
...
//                         <p className="text-sm text-gray-500 mt-1">
//                             Your {exportType === "pdf" ? "PDF" : "Excel"} report is ready
//                         </p>

//                         <p className="text-xs text-gray-400 mt-2">
//                             ✨ compliance-report-2026-01-17.
...
//                         Your compliance report includes all clauses, AI responses, and
//                         confidence scores
//                     </p>
//                 )}
//             </div>
...
const questions: Question[] = location.state?.questions?.map((row: any) => ({
  question_no: row.id,
  question: row.clause,
  answer: row.response,
  reference: row.reference ?? "-",
  score: row.score ?? getScoreFromAnswer(row.response)
})) ?? [];
...
const mappedRows: ResultRow[] = questions.map((q: any) => ({
  ...
  reference: q.reference ?? "-", // ✅ provide fallback
}));


  /* ---------------- PDF ---------------- */
  const generatePDF = (questions: Question[]) => {
    const doc = new jsPDF("landscape", "pt", "a4"); // ensure correct units
    doc.setFontSize(16);
  ...
  columnStyles: { 0: { cellWidth: 40 }, 1: { cellWidth: 200 }, 2: { cellWidth: 120 }, 3: { cellWidth: 180 }, 4: { cellWidth: 60 } },
});
  ...
  saveAs(data, "Compliance-Report.xlsx");
};
